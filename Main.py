...
def login():  # 실행 시,생년월일 값이 8자리인지 확인 후, 이름 일치 팝업을 출력한다. 사용자가 확인을 누르면 인터페이스를 초기화하는 버튼
    global userName, routineStr, routineList, timeList, currentTime
    currentTime = time.localtime(time.time())  # 접속 시간
...
CodersLab = Label(root, text="Team17 LYY KMS", font=("Helvetica", 8), background="white", foreground="blue")
CodersLab.pack(side="right", anchor="s")
...
def eng_dic():  # 오늘의 영단어를 보여주는 함수
    ...
    words = ['abort', 'absurd', 'accord', 'accumulate', 'bankrupt', 'blast', 'breed', 'brew', 'caption', 'cater',
             'cathedral', 'chamber', 'chore', 'commence', 'deficiency', 'deficit', 'dampen',
             'engrave', 'extract', 'exertion', 'expedite', 'facilitate', 'faculty', 'fatality', 'federal', 'guardian',
             'gulf', 'habitat', 'halt', 'haunt', 'headquarter', 'intervene', 'intrigue', 'judiciary', 'knack', 'knot',
...
def num1():  # '루틴 실행' 버튼 클릭 시 실행, 대기화면을 출력한다.
    ...
    def RoutineStart():
        ...
        def RoutineRecursion(i):
            ...
            if i == len(routineList):
                routineResultLab = Label(root, text='모닝루틴 실행 결과', font=("함초롬바탕", 30), background='white',
                                         foreground="black")
                routineResultLab.place(x=240, y=20)
                ...
                totalGoalTimeLab.place(x=300, y=100)
                ...
                totalElaspedTimeLab.place(x=300, y=140)
                ...
                routineLab.pack(pady=30)

                goalTimeLab = Label(root, font=("Helvetica", 30), text=f"목표시간 = {timeList[i]}", foreground="black")
                goalTimeLab.place(x=250, y=150)

                elaspedTimeLab = Label(root, font=("Helvetica", 30), text="경과시간 = 00:00", foreground="black")
                elaspedTimeLab.place(x=250, y=240)

                nextRoutineBtn = Button(root, text="다음", command=lambda: RoutineRecursion(i + 1), font=("함초롬바탕", 15),
                                        background='white', foreground="black")
                nextRoutineBtn.place(x=400, y=360)
    ...
    routinePrintTitle.config(font=("함초롬바탕", 30, "bold"))
    routinePrintTitle.place(x=345, y=40)
    ...
    routinePrintSubTitle.place(x=120, y=120)

    routineIntroduction = Label(root)  # 루틴명만 모두 출력
    routineIntroduction.config(text=routineStr, font=("함초롬바탕", 15), background="white", foreground="blue",
                               wraplength=550)
    routineIntroduction.place(x=150, y=240)

    RoutineStartButton = Button(root)
    RoutineStartButton.config(text="시작", command=RoutineStart, font=("함초롬바탕", 15), background='white',
                              foreground="black")
    RoutineStartButton.place(x=400, y=360)
...
def num2():  # '루틴 수정'버튼 클릭 시 실행되는 함수
    ...
    inputText.place(x=200, y=100)
    file.close()
    getTextBtn = Button(root, width="10", height="3", text="저장", command=get_text, background="white",
                        foreground="black")
...
def Eng_words():  # '오늘의 영단어'버튼 클릭 시 실행되는 함수
    ...
    menu3Title.pack(pady=20)
    menu3SubTitle = Label(root, text="영단어는 영어 뜻으로 외웠을 때 더 효과적이라고 합니다. 오늘의 영단어를 외워봅시다.", background='white',
                          foreground="black", font=("함초롬바탕", 15))
    menu3SubTitle.pack(pady=20)
    menu3Phrase = Label(root, text=engWords, font=("함초롬바탕", 20), wraplength=600)  # wraplength=int(),숫자만큼 줄이 차면 줄바꿈한다.
    menu3Phrase.pack(pady=20)
    recallBtn = Button(root)
    recallBtn.config(text="확인", font=("함초롬바탕", 15), background='white', foreground="black", command=menuPage_recall)
    recallBtn.place(x=400, y=380)
...
def num4():  # '명상의 시간' 버튼 클릭 시 시행되는 함수
    ...
    first = Radiobutton(root, font=("Helvetica", 13), width="10", height="3", text="Track-1", variable=r, value=1,
                        command=track1, foreground="black")
    second = Radiobutton(root, font=("Helvetica", 13), width="10", height="3", text="Track-2", variable=r, value=1,
                         command=track2, foreground="black")
    third = Radiobutton(root, font=("Helvetica", 13), width="10", height="3", text="Track-3", variable=r, value=1,
                        command=track3, foreground="black")
    fourth = Radiobutton(root, font=("Helvetica", 13), width="10", height="3", text="Track-4", variable=r, value=1,
                         command=track4, foreground="black")
    fifth = Radiobutton(root, font=("Helvetica", 13), width="10", height="3", text="Track-5", variable=r, value=1,
                        command=track5, foreground="black")
    ...
    notice = Label(root)
    notice.config(text="#루틴을 실행하며 음악을 듣고 싶다면 Stop 버튼을 누르지 않고 메인화면으로 돌아가면 됩니다. ", font=("함초롬바탕", 9), background="white",
                  foreground="black")
...
def num5():  # '사용시 주의사항' 버튼 클릭 시 시행되는 함수
    ...
    Title.place(x=295, y=40)
    text = Label(root)  # 루틴명만 모두 출력
    text.config(
        text="1. 루틴 실행시 뜨는 에러: 루틴 실행을 마치면 _tkinter.TclError: invalid command name 이라는 에러가 발생합니다. 해당 에러는 재귀함수를 강제 종료하며 발생하는 오류로 프로그램 실행에는 지장이 없으니 무시해도 됩니다."
             "\n\n2. 루틴 수정 이후 적용이 안될 때: 루틴 수정을 마치면 프로그램을 종료하고 재실행해야 루틴 실행에서 수정한 내용이 적용됩니다."
             "\n\n3. 루틴 수정 시 규칙: 루틴 수정시 무조건 >>>루틴 내용 = 00(분):00(초)<<<의 구조로 저장하셔야 합니다.", font=("함초롬바탕", 13),
        background="white", foreground="black", wraplength=600)
...
def menuPage_recall():  # 창 초기화 후, 메뉴 페이지를 호출한다.
    ...
    menuTitle.config(font=("함초롱 바탕", 25, "bold"))
    menuTitle.place(x=350, y=20)

    greetings = Label(root)
    greetings.config(text="%s" % Motivation_Message.motivationMessage, font=("함초롬바탕", 10,"bold"), background="white",
                     foreground="pink", wraplength=400)
    greetings.place(x=233, y=75)

    menu1 = Button(root, text="루틴 실행", background="grey", foreground="black", font=("함초롬바탕,15"), width=25, height=1,
                   command=num1)
    menu2 = Button(root, text="루틴 수정", background="grey", foreground="black", font=("함초롬바탕,15"), width=25, height=1,
                   command=num2)
    menu3 = Button(root, text="오늘의 영단어", background="grey", foreground="black", font=("함초롬바탕,15"), width=25, height=1,
                   command=Eng_words)
    menu4 = Button(root, text="명상의 시간", background="grey", foreground="black", font=("함초롬바탕,15"), width=25, height=1,
                   command=num4)
    menu5 = Button(root, text="사용시 주의사항", background="grey", foreground="black", font=("함초롬바탕,15"), width=25, height=1,
                   command=num5)
    menu6 = Button(root, text="종료", background="grey", foreground="black", font=("함초롬바탕,15"), width=25, height=1,
                   command=root.destroy)

    menu1.place(x=280, y=130)
    menu2.place(x=280, y=180)
    menu3.place(x=280, y=230)
    menu4.place(x=280, y=280)
    menu5.place(x=280, y=330)
    menu6.place(x=280, y=380)

    todayLab = Label(root, text="안녕하세요! 모닝 루틴 프로그램 '미라클 모닝'을 실행시켜주셔서 감사합니다😄"
                                f"\n{currentTime.tm_year}년 {currentTime.tm_mon}월 {currentTime.tm_mday}일 "
                                f"{currentTime.tm_hour}시 {currentTime.tm_min}분에 접속하셨습니다.")
    todayLab.config(font=("함초롱바탕", 12), background="white", foreground="black")
    todayLab.place(x=130, y=440)
